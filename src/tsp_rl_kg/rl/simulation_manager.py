...
class SimulationManager:
    def __init__(
        ...
        self.max_performance = max(
            gm.target_manager.target_route_energy for gm in self.game_managers
        )
    ...
    def insert_game_manager_sorted(self, game_manager):
        ...
        if energy > 0:
            index = next(
                (
                    i
                    for i, gm in enumerate(self.game_managers)
                    if energy < gm.target_manager.target_route_energy
                ),
                len(self.game_managers),
            )
    ...
    def should_advance_curriculum(self):
        ...
        current_max = self.game_managers[
            self.curriculum_indices[self.current_curriculum_index]
        ].target_manager.target_route_energy
        ...
        self.logger.info(
            f"Curriculum advancement check: Performance: {avg_performance:.2f}, "
    ...
    def print_energy_routes(self):
        self.logger.info(f"Number of environments: {len(self.game_managers)}")
        self.logger.info(
            f"Minimum energy route: {self.game_managers[0].target_manager.target_route_energy}"
        )
        self.logger.info(
            f"Maximum energy route: {self.game_managers[-1].target_manager.target_route_energy}"
        )
    ...
    def plot_curriculum(
        ...
        ax1.scatter(
            ...
            color="red",
            zorder=5,
            label="Selected Simulation Points",
            s=10,
        )
        ...
        ax2 = plt.twinx()
        ax2.set_ylabel(
            f"Curriculum step size: ~{self.step_size} energy units", fontsize=10, color="blue"
        )
        ax2.tick_params(
            right=False, labelright=False
        )  # Turn off ticks and tick labels for the secondary axis
    ...
    def create_plots(self, energy_values, curriculum_indices):
        ...
        """
        simulation_points = [
            energy_values[i] for i in curriculum_indices
        ]  # Energy at selected points

        # First plot with the environment index
        self.plot_curriculum(
            list(range(len(energy_values))),
            energy_values,
            curriculum_indices,
            simulation_points,
            xlabel="Environment index",
            ylabel="Energy required for trade route",
        ...
        x_values = list(
            range(len(curriculum_indices))
        )  # Use a simple list of indices for the x-values
        self.plot_curriculum(
            x_values,
            simulation_points,
            x_values,
            simulation_points,
            xlabel="Curriculum order index",
            ylabel="Energy required for trade route",
    ...
    def save_data(
        ...
        with open(game_data_file_path, mode="w", newline="") as game_data_file:
            ...
            for index in self.curriculum_indices:
                ...
                writer.writerow(
                    [index, target_route_energy, ",".join(map(str, achieved_route_energies))]
                )
