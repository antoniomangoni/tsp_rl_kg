...
class ModelTrainer:
    ...
    def train(self, total_timesteps, eval_callback, timeout=3600):
        ...
        if curriculum_callback.should_stop:
            self.logger.info(
                "Training ended early due to early stop condition", logger_name="training"
            )
        else:
    ...
    def log_training_stats(self):
        ...
        self.logger.info(
            f"Recent mean episode length: {mean_episode_length:.2f}", logger_name="training"
        )
        self.logger.info(
            f"Episode info buffer size: {len(self.rl_model.ep_info_buffer)}", logger_name="training"
        )
        if len(self.rl_model.ep_info_buffer) > 0:
            self.logger.info(
                f"Sample episode info: {self.rl_model.ep_info_buffer[-1]}", logger_name="training"
            )
        self.logger.info(
    ...
    def evaluate_model(self, eval_env, n_eval_episodes=10):
        ...
        self.logger.info(
            f"Final evaluation: Mean reward: {mean_reward:.2f} +/- {std_reward:.2f}",
            logger_name="eval",
        )
